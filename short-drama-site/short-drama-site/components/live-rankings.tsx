...
export function LiveRankings() {
  ...
  const load = useCallback(async () => {
    ...
    try { const response = await fetch(`/api/rankings?t=${Date.now()}`, { cache: "no-store" }); if (!response.ok) throw new Error("排行榜暂时无法更新"); const result = await response.json() as { panels?: Panel[] }; setPanels(result.panels ?? []); }
