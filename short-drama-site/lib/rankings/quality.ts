...
const rejectedContentPattern =
  /\b(trailer|review|recap|explanation|explained|clip|watch\s+free|full\s+movie|movie|episode\s+only|youtube\s+compilation|reaction|commentary|preview|teaser|fanmade|edit)\b|解说|讲解|盘点|吐槽|影评|预告|花絮|混剪|二创/i;
...
export function isEligibleRankingResource(resource: LiveSearchResource) {
  ...
  if (!resource.official_source) return false;
