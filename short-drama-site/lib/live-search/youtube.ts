...
export async function searchYouTube(query: string): Promise<LiveSearchResource[]> {
  ...
  return items.flatMap((item) => {
    ...
    if (rejectedContent.test(evidence) || !officialYouTubeChannelPattern.test(snippet.channelTitle ?? "")) return [];
    ...
    return [{
      ...
      verifiedOfficial: officialYouTubeChannelPattern.test(snippet.channelTitle ?? ""),
...
export async function discoverYouTubeShortDramas(): Promise<LiveSearchResource[]> {
  ...
  search.searchParams.set("q", "short drama full episode official English subtitles ReelShort DramaBox ShortMax GoodShort FlexTV NetShort");
  search.searchParams.set("key", apiKey);
  ...
  return items.flatMap((item) => {
    ...
    const evidence = `${snippet.title} ${snippet.description ?? ""} ${snippet.channelTitle ?? ""}`;
    if (rejectedContent.test(evidence) || !officialYouTubeChannelPattern.test(snippet.channelTitle ?? "")) return [];
    if (!/\b(short drama|full episode|reelshort|dramabox|netshort|shortmax|goodshort|flextv|english subtitles)\b/i.test(evidence)) return [];
    const detail = details.get(videoId);
