...
describe("search", () => {
  ...
  it("keeps the drama entity searchable without restoring removed disputed resources", () => {
    const result = searchDramas(dramas, platforms, { query: "The Lion's Captive" });
    expect(result[0]?.slug).toBe("the-lions-captive");
    expect(result[0]?.resources).toHaveLength(0);
  });
