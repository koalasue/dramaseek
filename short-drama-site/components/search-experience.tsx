...
export function SearchExperience({ dramas, platforms, initialQuery = "", initialPlatform = "all", embedded = false }: { dramas: Drama[]; platforms: Platform[]; initialQuery?: string; initialPlatform?: string; embedded?: boolean }) {
  ...
  return (
    <section aria-label="短剧搜索" className={embedded ? "mt-4 md:mt-5" : "page-shell py-5 md:py-6"}>
      ...
      {!liveLoading && visibleLive.length > 0 && <div className="mt-3 grid gap-2">{visibleLive.map((resource) => { const livePlatform = platforms.find((item) => item.id === resource.platformId); return <article key={resource.id} className="result-enter surface grid grid-cols-[70px_minmax(0,1fr)] gap-3 rounded-xl border line p-3 md:grid-cols-[90px_minmax(0,1fr)_auto] md:items-center">
        ...
        <div className="min-w-0"><div className="flex flex-wrap gap-1.5 text-[11px] text-muted"><span>{livePlatform?.name ?? "平台"}</span>{resource.durationSeconds != null && <span>{Math.round(resource.durationSeconds / 60)} 分钟</span>}<span>{resource.contentType === "full_series" ? "完整合集" : "剧集页面"}</span></div><h3 className="mt-1 line-clamp-2 text-sm font-semibold leading-5 md:text-base">{resource.title}</h3>{resource.description && <p className="mt-1 line-clamp-2 text-xs leading-5 text-muted">{resource.description}</p>}<p className="mt-1.5 truncate text-[11px] text-muted">来源：{resource.uploader}</p></div>
        <div className="col-span-2 flex justify-end md:col-span-1"><Link href={`/watch?url=${encodeURIComponent(resource.url)}&title=${encodeURIComponent(resource.title)}`} className="focus-ring pressable inline-flex min-h-10 items-center justify-center gap-1.5 whitespace-nowrap rounded-lg border line px-3 text-xs font-semibold">观看<ArrowSquareOut size={15}/></Link></div>
      </article>})}</div>}
