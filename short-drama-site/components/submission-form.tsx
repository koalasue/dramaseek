...
export function SubmissionForm() {
  ...
  return <form onSubmit={submit} className="surface rounded-2xl border line p-5 md:p-7">
    <div className="grid gap-5">
      <label className="grid gap-2 text-sm font-medium">正版资源链接<span className="text-xs font-normal text-muted">支持六个已接入平台的 HTTPS 页面，不接受视频文件或播放流直链。</span><input required name="url" type="url" placeholder="https://www.youtube.com/..." className="focus-ring surface-strong rounded-xl border line px-4 py-3 placeholder:text-[color:var(--muted)]" /></label>
      ...
      <label className="grid gap-2 text-sm font-medium">补充说明<span className="text-xs font-normal text-muted">请说明为什么它是官方或已授权来源。</span><textarea name="note" maxLength={800} rows={4} className="focus-ring surface-strong resize-y rounded-xl border line px-4 py-3" /></label>
