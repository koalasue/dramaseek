...
export function SearchExperience({ dramas, platforms, initialQuery = "", initialPlatform = "all", embedded = false }: { dramas: Drama[]; platforms: Platform[]; initialQuery?: string; initialPlatform?: string; embedded?: boolean }) {
  ...
  return (
    <section aria-label="短剧搜索" className={embedded ? "mt-8 md:mt-10" : "page-shell py-10 md:py-14"}>
      ...
      {!liveLoading && visibleLive.length > 0 && <div className="mt-4 grid gap-4">{visibleLive.map((resource) => { const livePlatform = platforms.find((item) => item.id === resource.platformId); return <article key={resource.id} className="result-enter surface grid overflow-hidden rounded-2xl border line sm:grid-cols-[140px_1fr] lg:grid-cols-[170px_1fr_auto]">
        <div className="relative aspect-[16/10] bg-[color:var(--surface-strong)] sm:aspect-auto sm:min-h-44">{resource.thumbnailUrl && <Image src={resource.thumbnailUrl} alt={`${resource.title} 封面`} fill sizes="(max-width: 640px) 100vw, 170px" className="object-cover" />}</div>
        <div className="p-4 md:p-5"><div className="flex flex-wrap gap-2 text-[11px] text-muted"><span>{livePlatform?.name ?? "平台"}</span>{resource.durationSeconds != null && <span>{Math.round(resource.durationSeconds / 60)} 分钟</span>}<span>{resource.contentType === "full_series" ? "完整合集" : "剧集页面"}</span></div><h3 className="mt-2 text-lg font-semibold tracking-tight md:text-xl">{resource.title}</h3>{resource.description && <p className="mt-2 line-clamp-3 text-sm leading-6 text-muted">{resource.description}</p>}<p className="mt-3 text-xs text-muted">来源：{resource.uploader}</p></div>
        <div className="flex items-end justify-end p-4 pt-0 sm:col-span-2 lg:col-span-1 lg:p-5"><Link href={`/watch?url=${encodeURIComponent(resource.url)}&title=${encodeURIComponent(resource.title)}`} className="focus-ring pressable inline-flex w-full items-center justify-center gap-2 whitespace-nowrap rounded-xl border line px-5 py-3 text-sm font-semibold sm:w-auto">前往观看<ArrowSquareOut size={17}/></Link></div>
      </article>})}</div>}
