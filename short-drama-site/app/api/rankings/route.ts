...
let cache: { expires: number; panels: Array<{ id: string; name: string; mode: string; entries: Entry[] }> } | null = null;

export async function GET() {
  if (cache && cache.expires > Date.now()) return NextResponse.json({ panels: cache.panels, updatedAt: new Date(cache.expires - 1_800_000).toISOString() });
  ...
  cache = { expires: Date.now() + 1_800_000, panels };
  return NextResponse.json({ panels, updatedAt: new Date().toISOString() }, { headers: { "Cache-Control": "public, s-maxage=1800, stale-while-revalidate=3600" } });
}
